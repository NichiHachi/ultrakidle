...
const StreakBadge = ({ streak }: { streak: number }) => (
  <div
    className={`text-base font-bold flex gap-0 flex-row ${
        ...
        <AnimatePresence initial={false}>
          {sortedUsers.map((user, index) => (
            <motion.div
              ...
              className={`flex items-start justify-center gap-1 p-2 border w-36 flex-shrink-0 transition-colors ${
                user.status === "won"
      ...
      <AnimatePresence initial={false}>
        {sortedUsers.map((user, index) => (
          <motion.div
            ...
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className={`flex items-start w-[135px] justify-center gap-1 p-2 border transition-colors ${
              user.status === "won"
                ? "bg-green-500/10 border-green-500/30"
                : user.status === "lost"
                  ? "bg-red-500/10 border-red-500/30"
