  ...
  for (const forum of forums) {
    ...
    for (const thread of newThreads) {
      try {
        ...
        if (!msg) {
          console.log(
            `[ingest] Archiving thread ${thread.id} — no starter message`,
          );
        ...
        console.error(
          `[ingest] Unexpected error for thread ${thread.id}:`,
          e,
        );
  ...
  if (pending?.length) {
    for (const sub of pending) {
      try {
        ...
        if (!levelId) {
          console.log(
            `[resolve] Rejecting #${sub.id} - title edited to invalid`,
          );
          await rejectThread(
            sub.channel_id,
            sub.message_id,
        ...
        console.error(
          `[resolve] Unexpected error for submission #${sub.id}:`,
          e,
        );
        await supabase
          .from("image_submissions")
