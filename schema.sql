...
CREATE OR REPLACE FUNCTION "public"."abandon_ig_cybergrind_run"() RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
...
begin
  ...
  select id, current_wave into v_run_id, v_current_wave
  from ig_cybergrind_runs
  ...
  from ig_cybergrind_rounds
  where run_id = v_run_id and round_number = v_current_wave;

  if v_round_rec.id is null then
    raise exception 'Round % not found for run %', v_current_wave, v_run_id;
  end if;

  if v_round_rec.completed_at is null then
    raise exception 'Current round must be completed before advancing.';
  end if;

  -- 1. Increment wave
  v_current_wave := v_current_wave + 1;
  update ig_cybergrind_runs set current_wave = v_current_wave where id = v_run_id;

  -- 2. Fetch new image for wave + 2 (Started_at for current wave handled by Edge Function)
  select s.id, s.storage_path, s.image_url, s.level_id, s.submitter_id into v_sub_record
  ...
  values (
    ...
    v_current_wave + 2, 
    v_sub_record.id, 
    v_sub_record.level_id, 
    v_sub_record.submitter_id
  )
  returning id into v_next_round_id;

  -- 3. Gather rounds for processing
  select coalesce(jsonb_agg(r_data), '[]'::jsonb) into v_rounds_to_process
  from (
    select 
      r.id as round_id, 
      r.round_number, 
    ...
    join image_submissions s on r.image_submission_id = s.id
    where r.run_id = v_run_id 
    and r.round_number in (v_current_wave, v_current_wave + 1, v_current_wave + 2)
...
begin
  ...
  if p_apply_grace_period then
    return greatest(0::numeric, p_time_spent_seconds - 0.8) * v_drain_rate;
  else
  ...
  then
    ...
    if v_waves_reached > 0 then
      ...
      if v_profile.channel_id is not null then
        ...
        v_msg := 'New Cybergrind (classic) record for ' || v_user_mention || '!'
...
CREATE OR REPLACE FUNCTION "public"."get_cybergrind_state"("p_version" "text" DEFAULT NULL::"text") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'auth'
    AS $$
declare
...
begin
  ...
  from cybergrind_records
  where user_id = caller_id
  order by best_wave desc, avg_accuracy desc
  limit 1;
    ...
    return json_build_object(
      ...
      'all_time_best', case when v_all_time_best is not null then
        json_build_object(
          ...
          'total_guesses', v_all_time_best.total_guesses,
          'avg_accuracy', v_all_time_best.avg_accuracy,
          'hint_accuracy', v_all_time_best.hint_accuracy,
          'client_version', v_all_time_best.client_version
        )
      else null end,
...
ALTER FUNCTION "public"."get_cybergrind_state"("p_version" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_ig_cybergrind_start_waves"() RETURNS integer[]
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'auth'
    AS $$
declare
...
ALTER FUNCTION "public"."get_ig_cybergrind_start_waves"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_ig_cybergrind_state"() RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'auth'
    AS $$
declare
...
begin
  ...
  if v_run is null or v_run.id is null then
    return json_build_object(
      'status', 'no_run',
      'start_waves', public.get_ig_cybergrind_start_waves(),
      'best', case when v_best is not null then
        json_build_object(
          'best_wave', v_best.best_wave,
          'avg_accuracy', v_best.avg_accuracy,
...
begin
  ...
  select json_agg(
    json_build_object(
      ...
      'submitted_by', json_build_object('name', sp.discord_name, 'avatar_url', sp.discord_avatar_url)
    ) order by idr.round_number
  ) into rounds_data
  from inferno_guesses ig
  join inferno_daily_rounds idr on ig.round_id = idr.id
  ...
  join submitter_profiles sp on sp.id = idr.submitter_id
  where idr.set_id = today_set_id and ig.user_id = caller_id;
  ...
  if completed_count >= 5 then
    ...
    select idr.id, idr.public_image_url, sp.discord_name, sp.discord_avatar_url
...
CREATE OR REPLACE FUNCTION "public"."get_submission_stats"() RETURNS json
    LANGUAGE "sql" STABLE
    AS $$
  select json_build_object(
    'total_approved', (select count(*) from image_submissions where status = 'approved'),
    'by_level', (
...
begin
  ...
  if today_set_id is null then
    ...
    if completed_count >= 5 then
      ...
      select idr.id, idr.public_image_url, sp.discord_name, sp.discord_avatar_url into current_round_row
      from inferno_daily_rounds idr 
      join submitter_profiles sp on sp.id = idr.submitter_id
...
begin
  ...
  begin
    if not exists (select 1 from inferno_daily_sets where game_date = today_nicaragua) then
      ...
      for submission in
        select id, level_id, submitter_id
        from image_submissions
        where status = 'approved'
          and id not in (
      ...
      loop
        ...
        insert into inferno_daily_rounds
          (set_id, round_number, image_submission_id, correct_level_id,
           public_image_url, submitter_id)
        values
          (new_set_id, round_num, submission.id, submission.level_id, '', submission.submitter_id);
      end loop;
...
v_max_start_wave :=
      (floor(coalesce(v_best_wave, 0)::numeric / 10) * 5)::int;
...
insert into ig_cybergrind_runs (user_id, current_wave, status, client_version, health)
  ...
  loop
    with inserted_round as (
      ...
      values (
        v_run_id, 
        start_wave + jsonb_array_length(v_rounds_created), 
...
begin
  ...
  -- Lock based on user and specific run to prevent duplicate processing
  perform pg_advisory_xact_lock(hashtext(caller_id::text || v_run.id::text)::bigint);
  ...
  if
    v_round.blessed_enemy_ids is not null and
    guess_id = ANY(v_round.blessed_enemy_ids)
  then
    v_is_blessed := true;
  end if;
  ...
  if not is_correct then
    if v_is_blessed then
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,enemy_type,result}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,weight_class,result}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,health,result}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,health,color}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,level_count,result}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,level_count,color}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
        '{properties,appearance,result}',
        'null'::jsonb
      );
      v_hint_data := jsonb_set(
        v_hint_data,
      ...
      v_hint_data := public.apply_hint_modifiers(
        v_hint_data,
        v_round.modifiers,
        v_round.eclipsed_column,
        v_round.radiance_targets
      );
    end if;
  end if;
  ...
  values (v_round.id, caller_id, guess_id, v_hint_data, false, v_is_blessed);
  ...
  if is_correct then
    ...
    if v_mod_count = 0 then
      ...
      with
        shuffled as materialized (
          select
            unnest(
              ARRAY['PENANCE', 'FALSIFIER', 'LETHE', 'ECLIPSE']::cybergrind_modifier_enum[]
            ) as m,
            random() as r
        )
      select array(select m from shuffled order by r limit 1)
      ...
      with
        shuffled as materialized (
          select
            unnest(
              ARRAY['PENANCE', 'FALSIFIER', 'LETHE', 'ECLIPSE', 'RADIANCE', 'IDOL']::cybergrind_modifier_enum[]
            ) as m,
            random() as r
        )
      select array(select m from shuffled order by r limit v_mod_count)
    ...
    if 'IDOL' = ANY(v_new_modifiers) then
      v_idol_count := case
        when 'IDOL' = ANY(v_new_radiance_targets) then 6
        else 3
      end;
      v_idol_rounds := case
        when 'IDOL' = ANY(v_new_radiance_targets) then 4
        else 2
      end;
      ...
      select array(
        ...
        from (
          ...
          from (
            ...
            where
              cr.run_id = v_run.id and
              cr.round_number >= (v_new_wave - v_idol_rounds)
    ...
    if 'PENANCE' = ANY(v_new_modifiers) then
      v_penance_count := case
        when 'PENANCE' = ANY(v_new_radiance_targets) then 2
        else 1
      end;
      select array(
        select id
    ...
    if v_new_penance_ids is not null then
      foreach v_pid in array v_new_penance_ids loop
        ...
        values (v_new_round_id, caller_id, v_pid, v_p_hint, true, false);
      end loop;
    end if;
  ...
  return json_build_object(
    ...
    json_build_object(
      ...
      v_state_round_id,
      'modifiers',
      to_json(v_state_round.modifiers),
      'eclipsed_column',
      v_state_round.eclipsed_column,
      'radiance_targets',
      to_json(v_state_round.radiance_targets),
      'guesses',
      v_state_guesses,
      'guess_count',
      v_state_guess_count,
...
begin
  ...
  -- Create a 64-bit hash for the advisory lock to serialize requests for this user/choice
  perform pg_advisory_xact_lock(hashtext(caller_id::text || current_choice_id::text)::bigint);

  insert into debug_logs (user_id, event, payload)
  values (
    caller_id,
    'guess_attempt_started',
    json_build_object('guess_id', guess_id)
  );

  select enemy_id
  into target_enemy_id
  from daily_choices
  ...
  into t_level_count
  from level_enemies
  where enemy_id = target_enemy_id;
  select count(*)
  into g_level_count
  from level_enemies
  where enemy_id = guess_id;

  is_correct := (target_enemy_id = guess_id);

  debut_distance := abs(
    coalesce(t_level_row.order_index, 0) - coalesce(g_level_row.order_index, 0)
  );
  ...
  response_data := json_build_object(
    ...
    json_build_object(
      ...
      json_build_object(
        ...
        'result',
        (case
          when t_row.enemy_type = g_row.enemy_type then 'correct'
          else 'incorrect'
        end)
      ),
      'weight_class',
      json_build_object(
        ...
        'result',
        (case
          when t_row.weight_class = g_row.weight_class then 'correct'
          else 'incorrect'
        end)
      ),
      'health',
      ...
      json_build_object(
        'value',
        g_row.first_appearance,
        'result',
        (case
          when t_row.debut_level_id = g_row.debut_level_id then 'correct'
          when
            coalesce(t_level_row.order_index, 0) >
            coalesce(g_level_row.order_index, 0)
          then
            'later'
          else
            'earlier'
  ...
  if not is_correct and guess_count = 4 then
    response_data := response_data::jsonb ||
      jsonb_build_object('correct_id', target_enemy_id);
  end if;
  ...
  values (
    ...
    ARRAY[
      (case
        when target_enemy_id = guess_id then 'GREEN'
        else 'RED'
      end)::hint_color,
      (case
        when t_row.enemy_type = g_row.enemy_type then 'GREEN'
        else 'RED'
      end)::hint_color,
      (case
        when t_row.weight_class = g_row.weight_class then 'GREEN'
        else 'RED'
      end)::hint_color,
      (case
        when t_row.health = g_row.health then 'GREEN'
  ...
  values (
    caller_id,
    'guess_success',
    json_build_object('is_correct', is_correct)
  );
...
CREATE OR REPLACE FUNCTION "public"."submit_ig_cybergrind_guess"("p_round_id" bigint, "p_guessed_level_id" bigint, "version" "text" DEFAULT NULL::"text") RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'auth', 'extensions'
    AS $$
declare
...
begin
  ...
  select * into v_run_row from ig_cybergrind_runs where id = v_round_row.run_id for update;
  ...
  if v_effective_hp <= 0 then
    ...
    -- Score is 100 * 0.85^distance
    v_computed_score := round(100.0 * power(0.85, v_computed_dist))::integer;

    if v_computed_dist = 0 then
      v_new_health := greatest(20, least(100, v_new_health + 20))::numeric;
    elsif v_computed_dist > 1 then
  ...
  return jsonb_build_object(
    ...
    'guessed_level', jsonb_build_object('id', p_guessed_level_id, 'level_number', v_guessed_level_info.level_number, 'level_name', v_guessed_level_info.level_name),
    'correct_level', jsonb_build_object('id', v_round_row.correct_level_id, 'level_number', v_correct_level_info.level_number, 'level_name', v_correct_level_info.level_name),
    'distance', v_computed_dist,
    'score', v_computed_score,
    'time_spent_seconds', v_computed_time_seconds,
...
ALTER FUNCTION "public"."submit_ig_cybergrind_guess"("p_round_id" bigint, "p_guessed_level_id" bigint, "version" "text") OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."cybergrind_leaderboard" AS
 SELECT "cr"."user_id",
    ...
    "cr"."achieved_at",
    "cr"."client_version",
    ("rank"() OVER (PARTITION BY "cr"."client_version" ORDER BY "cr"."best_wave" DESC, "cr"."avg_accuracy" DESC))::integer AS "rank"
   FROM ("public"."cybergrind_records" "cr"
     JOIN "public"."profiles" "p" ON (("p"."id" = "cr"."user_id")))
  WHERE ("cr"."best_wave" > 0);
...
CREATE TABLE IF NOT EXISTS "public"."ig_cybergrind_records" (
    "user_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."ig_cybergrind_records" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."ig_cybergrind_leaderboard" AS
 SELECT "cr"."user_id",
    "p"."discord_name",
    "p"."avatar_url",
    "cr"."best_wave",
    ...
    "cr"."client_version",
    ("rank"() OVER (PARTITION BY "cr"."client_version" ORDER BY "cr"."best_wave" DESC, "cr"."avg_accuracy" DESC))::integer AS "rank"
   FROM ("public"."ig_cybergrind_records" "cr"
     JOIN "public"."profiles" "p" ON (("p"."id" = "cr"."user_id")))
  WHERE ("cr"."best_wave" > 0);


ALTER VIEW "public"."ig_cybergrind_leaderboard" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."ig_cybergrind_rounds" (
    "id" bigint NOT NULL,
    "run_id" bigint NOT NULL,
    "round_number" integer DEFAULT 1 NOT NULL,
    "correct_level_id" bigint,
    "submitter_id" bigint,
    "image_submission_id" bigint,
...
ALTER TABLE "public"."ig_cybergrind_rounds" OWNER TO "postgres";


ALTER TABLE "public"."ig_cybergrind_rounds" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."ig_cybergrind_rounds_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS "public"."ig_cybergrind_runs" (
    "id" bigint NOT NULL,
    "user_id" "uuid" NOT NULL,
    "current_wave" integer DEFAULT 1 NOT NULL,
    "status" "public"."cybergrind_run_status_enum" DEFAULT 'active'::"public"."cybergrind_run_status_enum" NOT NULL,
...
ALTER TABLE "public"."ig_cybergrind_runs" OWNER TO "postgres";


ALTER TABLE "public"."ig_cybergrind_runs" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."ig_cybergrind_runs_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE "public"."submitter_profiles" ALTER COLUMN "id" ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME "public"."submitter_profiles_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY "public"."ig_cybergrind_rounds"
    ADD CONSTRAINT "ig_cybergrind_rounds_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."ig_cybergrind_runs"
    ADD CONSTRAINT "ig_cybergrind_runs_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."submitter_profiles"
    ADD CONSTRAINT "submitter_profiles_discord_user_id_key" UNIQUE ("discord_user_id");



ALTER TABLE ONLY "public"."submitter_profiles"
    ADD CONSTRAINT "submitter_profiles_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_user_guesses_daily_choice_first_guess" ON "public"."user_guesses" USING "btree" ("daily_choice_id", "user_id", "created_at");
...
ALTER TABLE ONLY "public"."ig_cybergrind_records"
    ADD CONSTRAINT "ig_cybergrind_records_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ig_cybergrind_rounds"
    ADD CONSTRAINT "ig_cybergrind_rounds_correct_level_id_fkey" FOREIGN KEY ("correct_level_id") REFERENCES "public"."levels"("id");
...
ALTER TABLE ONLY "public"."ig_cybergrind_rounds"
    ADD CONSTRAINT "ig_cybergrind_rounds_run_id_fkey" FOREIGN KEY ("run_id") REFERENCES "public"."ig_cybergrind_runs"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ig_cybergrind_rounds"
    ADD CONSTRAINT "ig_cybergrind_rounds_submitter_id_fkey" FOREIGN KEY ("submitter_id") REFERENCES "public"."submitter_profiles"("id");



ALTER TABLE ONLY "public"."ig_cybergrind_runs"
    ADD CONSTRAINT "ig_cybergrind_runs_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON UPDATE CASCADE ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."image_submissions"
    ADD CONSTRAINT "image_submissions_submitter_id_fkey" FOREIGN KEY ("submitter_id") REFERENCES "public"."submitter_profiles"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."inferno_daily_rounds"
    ADD CONSTRAINT "inferno_daily_rounds_submitter_id_fkey" FOREIGN KEY ("submitter_id") REFERENCES "public"."submitter_profiles"("id");
...
ALTER TABLE "public"."ig_cybergrind_records" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."ig_cybergrind_rounds" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."ig_cybergrind_runs" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."abandon_ig_cybergrind_run"() TO "anon";
GRANT ALL ON FUNCTION "public"."abandon_ig_cybergrind_run"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."abandon_ig_cybergrind_run"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_cybergrind_state"("p_version" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_cybergrind_state"("p_version" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_cybergrind_state"("p_version" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_start_waves"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_start_waves"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_start_waves"() TO "service_role";



GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_state"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_state"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_ig_cybergrind_state"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_submission_stats"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."start_ig_cybergrind_run"("version" "text", "start_wave" integer, "caller_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."start_ig_cybergrind_run"("version" "text", "start_wave" integer, "caller_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."start_ig_cybergrind_run"("version" "text", "start_wave" integer, "caller_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."start_ig_cybergrind_run"("version" "text", "start_wave" integer, "caller_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."submit_ig_cybergrind_guess"("p_round_id" bigint, "p_guessed_level_id" bigint, "version" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."submit_ig_cybergrind_guess"("p_round_id" bigint, "p_guessed_level_id" bigint, "version" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."submit_ig_cybergrind_guess"("p_round_id" bigint, "p_guessed_level_id" bigint, "version" "text") TO "service_role";
...
GRANT ALL ON TABLE "public"."ig_cybergrind_records" TO "anon";
GRANT ALL ON TABLE "public"."ig_cybergrind_records" TO "authenticated";
GRANT ALL ON TABLE "public"."ig_cybergrind_records" TO "service_role";



GRANT ALL ON TABLE "public"."ig_cybergrind_leaderboard" TO "anon";
GRANT ALL ON TABLE "public"."ig_cybergrind_leaderboard" TO "authenticated";
GRANT ALL ON TABLE "public"."ig_cybergrind_leaderboard" TO "service_role";



GRANT ALL ON TABLE "public"."ig_cybergrind_rounds" TO "anon";
GRANT ALL ON TABLE "public"."ig_cybergrind_rounds" TO "authenticated";
GRANT ALL ON TABLE "public"."ig_cybergrind_rounds" TO "service_role";



GRANT ALL ON SEQUENCE "public"."ig_cybergrind_rounds_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."ig_cybergrind_rounds_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."ig_cybergrind_rounds_id_seq" TO "service_role";



GRANT ALL ON TABLE "public"."ig_cybergrind_runs" TO "anon";
GRANT ALL ON TABLE "public"."ig_cybergrind_runs" TO "authenticated";
GRANT ALL ON TABLE "public"."ig_cybergrind_runs" TO "service_role";



GRANT ALL ON SEQUENCE "public"."ig_cybergrind_runs_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."ig_cybergrind_runs_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."ig_cybergrind_runs_id_seq" TO "service_role";
...
GRANT ALL ON SEQUENCE "public"."submitter_profiles_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."submitter_profiles_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."submitter_profiles_id_seq" TO "service_role";
